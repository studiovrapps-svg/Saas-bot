...
replacement = r"""if (tenant.bot_tier === 1) {
                // Manejo de estado de pedido (Opcin 3 y fotos de producto)
                ...
                if (state.muted_until && Date.now() < state.muted_until) {
                    return res.sendStatus(200); // Ignorar mientras est en silencio
                }

                // Helper para enviar men principal interactivo
                const sendMainMenu = async () => {
                    ...
                    let payload = {
                        ...
                        interactive: {
                            ...
                            body: { text: tenant.tier1_greeting || `Hola! Bienvenido a ${tenant.name}. Cmo podemos ayudarte hoy?` },
                            action: {
                                button: "Ver opciones 👇",
                ...
                if (msgObj.type === "text") {
                    ...
                    // --- PILAR 1: MQUINA DE ESTADOS (CARRITO) ---
                    if (state.step === 'awaiting_quantity') {
                        ...
                        await sendInteractiveButtons("🛒 *Producto aadido al carrito.*\n\nDeseas seguir comprando o finalizar tu pedido?", [
                    ...
                    if (matchedRule) {
                        ...
                        await sendInteractiveButtons("Puedo ayudarte con algo ms?", [
                            { id: "btn_main_menu", title: "🔙 Men Principal" }
                        ]);
                        return res.sendStatus(200);
                    }

                    // --- PILAR 3: RUTAS DE ESCAPE (FALLBACK ANTI-FRUSTRACIN) ---
                    // Si no es un estado, ni keyword, ni handoff, dar mensaje de fallback amigable
                    chatCache.del(cacheKey);
                    await sendWhatsAppText(phone_number_id, tenant.whatsapp_token, from, "No te comprend muy bien 😅. Para ayudarte rpido, por favor selecciona una de nuestras opciones:");
                    await sendMainMenu();
                    return res.sendStatus(200);
                }
                ...
                if (msgObj.type === "interactive") {
                    ...
                    if (btnId === "btn_main_menu" || btnId === "btn_catalogo") {
                        ...
                        await sendWhatsAppText(phone_number_id, tenant.whatsapp_token, from, "Excelente! 📍 A qu direccin deseas que hagamos el envo de tu pedido completo?");
                        ...
                        if (menus[idx]) {
                            ...
                            // Siempre mandar el escape despus de un FAQ
                            await sendInteractiveButtons("Qu ms deseas hacer?", [
                                { id: "btn_main_menu", title: "🔙 Men Principal" }
                            ]);
                        }
                        ...
                        await sendWhatsAppText(phone_number_id, tenant.whatsapp_token, from, `Excelente eleccin! 🌟\n\nCuntas unidades deseas llevar? (Responde con un nmero)`);
                    } else {
                        // Fallback fallback
                        await sendMainMenu();
                    }
