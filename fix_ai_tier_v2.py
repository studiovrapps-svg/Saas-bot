...
# Fix the regex to stop exactly at the end of sendWhatsAppAI, right before "// --- RUTAS DE LA API"
old_sendWhatsAppAI_pattern = re.compile(r'async function sendWhatsAppAI\(phone_number_id, token, to, tenant_id, tenant_name, system_prompt, user_message\) \{.*?(?=// --- RUTAS DE LA API)', re.DOTALL)
...
code = old_sendWhatsAppAI_pattern.sub(lambda m: new_sendWhatsAppAI, code)
