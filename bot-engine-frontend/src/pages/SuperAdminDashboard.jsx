...
function SuperAdminDashboard() {
  ...
  const [activeAdminTab, setActiveAdminTab] = useState(() => {
    return localStorage.getItem('superAdminActiveTab') || 'metricas';
  });

  useEffect(() => {
    localStorage.setItem('superAdminActiveTab', activeAdminTab);
  }, [activeAdminTab]);
  ...
  return (
    <div className="flex h-screen bg-slate-100 font-sans text-gray-900 overflow-hidden relative">
      ...
      <div className={`w-64 bg-white border-r border-gray-200 flex flex-col z-30 absolute inset-y-0 left-0 transform transition-transform duration-300 md:relative md:translate-x-0 ${showMobileMenu ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="px-6 pt-6 pb-2 border-b border-gray-200 flex items-center justify-start">
          <div className="flex flex-col items-center ml-2">
            <img src="/logo-dynova.jpeg" alt="Dynova Logo" className="h-20 w-auto object-contain -mb-4" />
            <p className="text-gray-500 text-xs font-bold tracking-wider uppercase mt-0">Super Admin</p>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {activeAdminTab === "metricas" && adminStats && !adminStats.error && (
                <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
                        <div onClick={() => setBreakdownModal({ isOpen: true, type: 'mrr' })} className="bg-white p-4 md:p-6 rounded-xl shadow-md border border-gray-200 flex flex-col cursor-pointer hover:shadow-md transition-shadow hover:border-blue-200">
                        ...
                        <div onClick={() => setBreakdownModal({ isOpen: true, type: 'cost' })} className="bg-white p-4 md:p-6 rounded-xl shadow-md border border-gray-200 flex flex-col cursor-pointer hover:shadow-md transition-shadow hover:border-blue-200">
                            <div className="flex items-center gap-2 mb-2 md:mb-3">
                        ...
                        <div onClick={() => setBreakdownModal({ isOpen: true, type: 'margin' })} className="bg-white p-4 md:p-6 rounded-xl shadow-md border border-gray-200 flex flex-col cursor-pointer hover:shadow-md transition-shadow hover:border-blue-200">
                            <div className="flex items-center gap-2 mb-2 md:mb-3">
                        ...
                        <div onClick={() => setBreakdownModal({ isOpen: true, type: 'interactions' })} className="bg-white p-4 md:p-6 rounded-xl shadow-md border border-gray-200 flex flex-col cursor-pointer hover:shadow-md transition-shadow hover:border-blue-200">
                            <div className="flex items-center gap-2 mb-2 md:mb-3">
                                ...
                                <div className="h-72 w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={adminStats.daily_usage} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                            ...
                                            <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} formatter={(value) => [`Q ${Number(value || 0).toFixed(2)}`, "Costo"]} />
                                            <Area type="monotone" dataKey="cost_gtq" name="Costo (Q)" stroke="#000000" strokeWidth={3} fillOpacity={1} fill="url(#colorCost)" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            ) : (
                                <div className="h-72 w-full flex items-center justify-center bg-gray-50 rounded-xl border border-gray-200">
                                        ...
                                        <div key={idx} className="flex justify-between items-center p-3 hover:bg-gray-50 rounded-lg transition">
                                            ...
                                            <div className="flex flex-col items-end">
                                                <div className="font-bold text-gray-900 bg-gray-100 px-3 py-1 rounded-full text-sm">
                                                    Q {Number(t.cost_gtq || 0).toFixed(2)}
                                                </div>
                                                <p className="text-[10px] text-gray-400 mt-1">USD {Number(t.cost_usd || 0).toFixed(4)}</p>
                                            </div>
                                        </div>
                                    ))
                                )}
            ...
            {activeAdminTab === "inquilinos" && (
                <>
                    <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center">
                        ...
                        <button onClick={() => setShowModal(true)} className="mt-4 md:mt-0 w-full md:w-auto bg-black text-white px-5 py-2.5 rounded-xl hover:bg-gray-800 font-semibold shadow-md transition text-sm flex items-center justify-center md:justify-start gap-2">
                    ...
                    <div className="hidden md:block bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
                        {filteredClientes.length === 0 ? (
                            <div className="p-16 flex flex-col items-center justify-center text-center bg-gray-50/50">
                                <div className="bg-white p-5 rounded-full shadow-sm mb-4 border border-gray-100">
                                    <svg className="w-10 h-10 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                </div>
                                <h3 className="text-lg font-bold text-gray-900 mb-2">No se encontraron clientes</h3>
                                <p className="text-gray-500 text-sm max-w-sm mb-6">No hay inquilinos registrados en la categoría <span className="font-semibold text-gray-700">"{tenantFilter}"</span>. Ajusta el filtro o añade uno nuevo.</p>
                                <button onClick={() => { setEditData(null); setFormData({name:"", email:"", password:"", bot_tier:1, monthly_price:0, template_id: "", features: {}}); setShowModal(true); }} className="bg-black text-white px-6 py-2.5 rounded-full font-semibold text-sm hover:bg-gray-800 transition flex items-center gap-2">
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
                                    Añadir Inquilino
                                </button>
                            </div>
                      ...
                      <div className="md:hidden flex flex-col gap-4">
                          {filteredClientes.length === 0 ? (
                              <div className="p-12 flex flex-col items-center justify-center text-center bg-white rounded-xl border border-gray-200 shadow-sm">
                                  <div className="bg-gray-50 p-4 rounded-full mb-4">
                                      <svg className="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                  </div>
                                  <h3 className="text-base font-bold text-gray-900 mb-1">No se encontraron clientes</h3>
                                  <p className="text-gray-500 text-xs max-w-[250px] mb-5">No hay inquilinos en <span className="font-semibold text-gray-700">"{tenantFilter}"</span>.</p>
                                  <button onClick={() => { setEditData(null); setFormData({name:"", email:"", password:"", bot_tier:1, monthly_price:0, template_id: "", features: {}}); setShowModal(true); }} className="bg-black text-white px-5 py-2 rounded-full font-semibold text-sm hover:bg-gray-800 transition flex items-center gap-2">
                                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
                                      Añadir
                                  </button>
                              </div>
                              ...
                              filteredClientes.map(c => (
                                  <div key={c.id} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col gap-3 shadow-md relative">
            ...
            {activeAdminTab === "plantillas" && (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {templates.length === 0 ? (
                            <div className="col-span-full bg-white rounded-3xl border-2 border-dashed border-gray-200 p-12 md:p-16 flex flex-col items-center justify-center text-center mt-4">
                                <div className="bg-gray-50 p-5 rounded-full mb-5 border border-gray-100">
                                    <svg className="w-10 h-10 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>
                                </div>
                                <h3 className="text-xl font-bold text-gray-900 mb-2">No hay plantillas creadas</h3>
                                <p className="text-gray-500 text-sm max-w-[350px] mb-8">Crea configuraciones base con reglas e instrucciones para desplegar rápidamente nuevos bots de industrias similares.</p>
                                <button onClick={() => { setEditTemplateData(null); setTemplateFormData({ name: "", bot_tier: 1, system_prompt: "", business_rules: "[]" }); setShowTemplateModal(true); }} className="bg-black text-white px-7 py-3 rounded-full font-semibold text-sm hover:bg-gray-800 shadow-md transition flex items-center gap-2">
                                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
                            ...
                            templates.map(t => (
                                <div key={t.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-md relative group hover:border-black transition flex flex-col">
            ...
            {activeAdminTab === "precios" && (
                <div className="max-w-6xl mx-auto">
                    ...
                    <form onSubmit={handleSavePricing} className="bg-white p-8 rounded-2xl shadow-md border border-gray-200">
                        
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                            {/* Columna Izquierda: Planes Base */}
                            <div>
                                <div className="flex items-center gap-2 mb-6 border-b pb-3">
                                    <h3 className="text-xl font-bold text-gray-900">Planes Base</h3>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <label className="font-bold text-gray-800 block text-base">Nivel 1</label>
                                            <span className="text-xs text-gray-500">Respuestas automatizadas básicas y menú simple.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.plans?.[1] || 0} onChange={e => setGlobalPricing({...globalPricing, plans: {...globalPricing.plans, 1: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <label className="font-bold text-gray-800 block text-base">Nivel 2</label>
                                            <span className="text-xs text-gray-500">Flujo completo de tienda, carrito y multi-agente.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.plans?.[2] || 0} onChange={e => setGlobalPricing({...globalPricing, plans: {...globalPricing.plans, 2: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <label className="font-bold text-gray-800 block text-base">Nivel 3</label>
                                            <span className="text-xs text-gray-500">Funciones avanzadas, integraciones e inteligencia artificial.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.plans?.[3] || 0} onChange={e => setGlobalPricing({...globalPricing, plans: {...globalPricing.plans, 3: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                </div>
                            </div>

                            {/* Columna Derecha: Módulos Extra */}
                            <div>
                                <div className="flex items-center gap-2 mb-6 border-b pb-3">
                                    <h3 className="text-xl font-bold text-gray-900">Módulos Extra</h3>
                                </div>
                                
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <span className="font-bold text-gray-800 block text-base">Gestor de Pedidos</span>
                                            <span className="text-xs text-gray-500">Panel para gestionar órdenes, estados y clientes.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.modules?.orders || 0} onChange={e => setGlobalPricing({...globalPricing, modules: {...globalPricing.modules, orders: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <span className="font-bold text-gray-800 block text-base">Bandeja Multi-Agente</span>
                                            <span className="text-xs text-gray-500">Varios humanos respondiendo chats.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.modules?.inbox || 0} onChange={e => setGlobalPricing({...globalPricing, modules: {...globalPricing.modules, inbox: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <span className="font-bold text-gray-800 block text-base">CRM Automático</span>
                                            <span className="text-xs text-gray-500">Base de datos de leads y perfiles.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.modules?.crm || 0} onChange={e => setGlobalPricing({...globalPricing, modules: {...globalPricing.modules, crm: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors">
                                        <div>
                                            <span className="font-bold text-gray-800 block text-base">Campañas Masivas</span>
                                            <span className="text-xs text-gray-500">Envío de difusiones y promociones.</span>
                                        </div>
                                        <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-black transition-all">
                                            <span className="bg-gray-100 px-3 py-2 text-gray-500 font-bold border-r border-gray-200">Q</span>
                                            <input type="number" required value={globalPricing.modules?.campaigns || 0} onChange={e => setGlobalPricing({...globalPricing, modules: {...globalPricing.modules, campaigns: Number(e.target.value)}})} className="w-24 p-2 text-right outline-none font-bold text-gray-900" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mt-10 border-t border-gray-100 pt-8 flex justify-end">
                            <button type="submit" disabled={loading} className="px-8 py-3 bg-black text-white rounded-xl font-bold text-sm hover:bg-gray-800 transition shadow-md flex items-center gap-2">
                        ...
                        <div className="overflow-auto flex-1">
                            <table className="w-full text-left border-collapse text-sm">
                                <thead className="bg-gray-50 sticky top-0 border-b border-gray-200 z-10">
                                    <tr>
                                        <th className="px-4 py-2.5 font-bold text-gray-500 uppercase tracking-wider text-xs whitespace-nowrap">Fecha</th>
                                        <th className="px-4 py-2.5 font-bold text-gray-500 uppercase tracking-wider text-xs whitespace-nowrap">Nivel</th>
                                        <th className="px-4 py-2.5 font-bold text-gray-500 uppercase tracking-wider text-xs whitespace-nowrap">Inquilino</th>
                                        <th className="px-4 py-2.5 font-bold text-gray-500 uppercase tracking-wider text-xs whitespace-nowrap">Tipo</th>
                                        <th className="px-4 py-2.5 font-bold text-gray-500 uppercase tracking-wider text-xs w-full">Mensaje</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {systemLogs.length === 0 ? (
                                        <tr>
                                            <td colSpan="5" className="p-16">
                                                <div className="flex flex-col items-center justify-center text-center">
                                                    ...
                                                    <p className="text-gray-900 font-bold mb-1">Sin registros en el sistema</p>
                                                    <p className="text-gray-500 text-sm">No se han detectado errores ni advertencias recientes.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                        systemLogs.map(log => (
                                              <tr key={log.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                                                  <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap text-xs">{new Date(log.created_at).toLocaleString()}</td>
                                                  <td className="px-4 py-2.5 whitespace-nowrap">
                                                      <span className={`px-2 py-0.5 rounded text-[10px] uppercase tracking-wider font-bold border ${log.level === 'ERROR' ? 'bg-red-50 text-red-600 border-red-100' : log.level === 'WARNING' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>
                                                          {log.level}
                                                      </span>
                                                  </td>
                                                  <td className="px-4 py-2.5 font-semibold text-gray-900 whitespace-nowrap">{log.tenant_name || 'Sistema'}</td>
                                                  <td className="px-4 py-2.5 text-gray-500 font-mono text-[11px] whitespace-nowrap">{log.event_type}</td>
                                                  <td className="px-4 py-2.5 w-full">
                                                      <div className="text-gray-900 font-medium text-sm">{log.message}</div>
                                                      {log.details && Object.keys(log.details).length > 0 && (
                                                          <details className="mt-1.5 group">
                                                              <summary className="text-[10px] font-bold text-gray-400 cursor-pointer hover:text-gray-700 flex items-center gap-1 select-none transition-colors w-max">
                                                                  <svg className="w-3 h-3 transition-transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"/></svg>
                                                                  Ver Payload JSON
                                                              </summary>
                                                              <pre className="mt-1.5 text-[10px] text-gray-600 bg-white p-2.5 rounded overflow-x-auto max-h-32 overflow-y-auto custom-scrollbar border border-gray-100 shadow-sm">
      ...
      {breakdownModal.isOpen && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50  flex items-center justify-center z-50 p-4" onClick={() => setBreakdownModal({ isOpen: false, type: null })}>
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl w-full max-w-2xl border border-gray-200 max-h-[95vh] overflow-y-auto scrollbar-hide" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50  flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-gray-200 max-h-[95vh] overflow-y-auto scrollbar-hide" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleCreate}>
                    ...
                    <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full border border-gray-200 p-2.5 rounded-xl bg-gray-50 mb-5 outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md text-sm" placeholder="Ej: CDS Premium" />
                    
                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Correo (Usuario)</label>
                    <input type="email" required value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full border border-gray-200 p-2.5 rounded-xl bg-gray-50 mb-5 outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md text-sm" placeholder="cliente@correo.com" />
                    
                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Contraseña Temporal</label>
                    <input type="text" required value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} className="w-full border border-gray-200 p-2.5 rounded-xl bg-gray-50 mb-6 outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md text-sm" placeholder="123456" />
      ...
      {showEditModal && editData && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50  flex items-center justify-center z-50 p-4" onClick={() => setShowEditModal(false)}>
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-lg border border-gray-200 max-h-[95vh] overflow-y-auto scrollbar-hide" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleEdit} autoComplete="off">
                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase tracking-wider">Nombre Comercial</label>
                    <input type="text" value={editData.name} onChange={e => setEditData({...editData, name: e.target.value})} className="w-full border border-gray-200 p-2.5 rounded-xl bg-gray-50 mb-5 outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md text-sm" />
                    ...
                    <div className="bg-gray-50 p-5 rounded-xl mb-6 border border-gray-200">
                        ...
                        {(!unlockApi && editData.whatsapp_phone_id) ? (
                            <div className="text-sm text-gray-500 italic flex items-center gap-2 bg-white p-4 rounded-lg border border-gray-200 shadow-md">
                            ...
                            <div className="animate-fade-in space-y-4">
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider">ID del Teléfono (Phone ID)</label>
                                        <div className="flex gap-1">
                                            <button type="button" onClick={async () => { try { const text = await navigator.clipboard.readText(); setEditData({...editData, whatsapp_phone_id: text.trim()}); } catch(e) { console.error('Error al pegar', e); } }} className="p-1 text-gray-400 hover:text-gray-900 hover:bg-gray-200 rounded transition" title="Pegar">
                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                                            </button>
                                            <button type="button" onClick={() => setEditData({...editData, whatsapp_phone_id: ''})} className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition" title="Borrar">
                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            </button>
                                        </div>
                                    </div>
                                    <input type="text" autoComplete="off" data-lpignore="true" data-form-type="other" value={editData.whatsapp_phone_id || ""} onChange={e => setEditData({...editData, whatsapp_phone_id: e.target.value})} className="w-full font-mono border border-gray-200 p-2.5 rounded-xl bg-gray-50 text-sm outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md" placeholder="Ej: 10423456789" />
                                </div>
                                
                                <div>
                                    <div className="flex justify-between items-center mb-1">
                                        <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider">Token de Acceso Permanente</label>
                                        <div className="flex gap-1">
                                            <button type="button" onClick={async () => { try { const text = await navigator.clipboard.readText(); setEditData({...editData, whatsapp_token: text.trim()}); } catch(e) { console.error('Error al pegar', e); } }} className="p-1 text-gray-400 hover:text-gray-900 hover:bg-gray-200 rounded transition" title="Pegar">
                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                                            </button>
                                            <button type="button" onClick={() => setEditData({...editData, whatsapp_token: ''})} className="p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition" title="Borrar">
                                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            </button>
                                        </div>
                                    </div>
                                    <input type="password" autoComplete="new-password" data-lpignore="true" data-form-type="other" value={editData.whatsapp_token || ""} onChange={e => setEditData({...editData, whatsapp_token: e.target.value})} className="w-full font-mono border border-gray-200 p-2.5 rounded-xl bg-gray-50 text-sm outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md" placeholder="EAAD... " />
                                </div>
                            </div>
                        )}
                    </div>
      ...
      {/* Modal Plantilla */}
      {showTemplateModal && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-50  flex items-center justify-center z-50 p-4" onClick={() => setShowTemplateModal(false)}>
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-lg border border-gray-200 max-h-[95vh] overflow-y-auto scrollbar-hide" onClick={(e) => e.stopPropagation()}>
                ...
                <form onSubmit={handleCreateTemplate}>
                    ...
                    <input type="text" required value={templateFormData.name} onChange={(e) => setTemplateFormData({...templateFormData, name: e.target.value})} className="w-full border border-gray-200 p-2.5 rounded-xl bg-gray-50 mb-5 outline-none focus:border-black focus:ring-1 focus:ring-black transition shadow-md text-sm" placeholder="Ej: Clínica Dental Base" />
