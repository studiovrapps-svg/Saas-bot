...
# 1. Remove the "Conexion" sidebar button
sidebar_pattern = r'<div onClick=\{\(\) => setActiveTab\(\'conexion\'\)\}.*?>[\s\S]*?</div>\s*</div>\s*<div className="p-4 border-t border-gray-100">'
code = re.sub(sidebar_pattern, r'</div>\n          <div className="p-4 border-t border-gray-100">', code)

# 2. Extract the connection UI logic
# Instead of complex regex extraction, I will just recreate the configuration block with the Meta Login UI and remove the old 'conexion' block.

old_conexion_ui = r'\{activeTab === \'conexion\' && \([\s\S]*?\}\)[\s\S]*?\{activeTab === \'dashboard\' && \('
code = re.sub(old_conexion_ui, r'{activeTab === \'dashboard\' && (', code)

old_config_ui = r'\{activeTab === \'configuracion\' && \([\s\S]*?\}\)'

new_config_ui = """{activeTab === 'configuracion' && (
...
code = re.sub(old_config_ui, new_config_ui, code)
