...
let content = fs.readFileSync('temp_ai.js', 'utf-8');
...
let newPromptLines = `        // Inyectamos la personalidad universal para TODO bot Tier 2
        ...
        if (customer_name) {
            sysPrompt += \`- El cliente con el que estás hablando ya está registrado con el nombre: \${customer_name}. Dirígete a él por su nombre para darle una atención cálida y personalizada.\\n\\n\`;
        } else {
...
content = content.replace(
    /\/\/ Inyectamos la personalidad universal para TODO bot Tier 2[\s\S]*?- Si el cliente acaba de saludar por primera vez.*Queremos que la charla sea súper humana\.\\n\\n`;/,
...
fs.writeFileSync('src/services/ai.service.js', content);
